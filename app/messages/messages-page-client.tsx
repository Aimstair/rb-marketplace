...
import { buildMessagingChannel, buildPrivateChatChannel } from "@/lib/pusher-channels"
import {
  getTransactionById,
...
function mapServerMessageToUi(msg: MessageData, userId?: string): Message {
  ...
  return {
    ...
    type: isCounteroffer ? "counteroffer" : msg.attachmentUrl ? "image" : "text",
...
function mapRealtimeMessageToUi(data: any, userId?: string): Message {
  return {
    id: data.id,
    sender: data.senderId === userId ? "buyer" : "seller",
    text: data.content,
...
const CONTACT_ITEM_ESTIMATED_HEIGHT = 112
const CONTACT_OVERSCAN = 6
const MESSAGE_ITEM_ESTIMATED_HEIGHT = 96
const MESSAGE_OVERSCAN = 10
...
export default function MessagesPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "ongoing" | "completed" | "sold" | "cancelled">("all")
  ...
  const [contactWindow, setContactWindow] = useState({ start: 0, end: 30 })
  const [messageWindow, setMessageWindow] = useState({ start: 0, end: 120 })
  const realtimeChatUnavailableRef = useRef(false)
  const realtimeMessagingUnavailableRef = useRef(false)
  const latestContactMessageIdRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    if (amountParam) {
      setAmountQueryParam(parseInt(amountParam, 10))
    }
  }, [amountParam])
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      ...
      try {
        ...
        const listingType = typeParam === "currency" ? "CURRENCY" : "ITEM"
        ...
        if (sellerIdParam) {
          try {
            ...
            const convResult = await getOrCreateConversation(sellerIdParam, itemIdParam || undefined, txDetails, listingType)
            ...
            if (convResult.success && convResult.conversationId) {
              ...
              if (messagesResult.success && messagesResult.messages && messagesResult.messages.length === 0) {
                ...
                const itemName = itemIdParam ? "this item" : "your listing"
        ...
        if (result.success && result.conversations) {
          ...
          if (sellerIdParam) {
            const targetContact = convertedContacts.find((c) => {
              return c.otherUserId === sellerIdParam && (itemIdParam ? c.item.id === itemIdParam : true)
            })
    ...
    loadConversations()
  }, [user?.id, isAuthLoading, sellerIdParam, itemIdParam, typeParam, amountParam, costParam])
  ...
  useEffect(() => {
    ...
    latestContactMessageIdRef.current = null
    const bufferedEvents: any[] = []
    const chatChannelName = buildPrivateChatChannel(selectedConversationId)
    const realtimeEnabled = isRealtimeMessagingFeatureEnabled() && !realtimeChatUnavailableRef.current
    const pusher = realtimeEnabled ? getPusherClient() : null

    const sortContactsByRecency = (list: Contact[]) => {
      list.sort((a, b) => {
        const dateA = new Date(a._sortDate || 0).getTime()
        const dateB = new Date(b._sortDate || 0).getTime()
        return dateB - dateA
    ...
    const markConversationReadLocally = () => {
      ...
      setSelectedContact((prev) =>
        prev && prev.id === selectedConversationId ? { ...prev, unreadCount: 0 } : prev
      )
    }

    const updateContactPreview = (message: Message, createdAt: Date) => {
      setContacts((prev) => {
        let changed = false
        const updated = prev.map((contact) => {
          ...
          const nextLastMessage = message.type === "image" ? "Sent a photo." : message.text
          const nextTimestamp = formatContactTimestamp(createdAt)
          const currentSortDate = new Date(contact._sortDate || 0).getTime()
          const nextSortDate = createdAt.getTime()

          if (
            contact.lastMessage === nextLastMessage &&
            contact.timestamp === nextTimestamp &&
            currentSortDate === nextSortDate
          ) {
            return contact
          }
        ...
        return sortContactsByRecency(updated)
      })
      ...
      setSelectedContact((prev) => {
        ...
        const nextLastMessage = message.type === "image" ? "Sent a photo." : message.text
        const nextTimestamp = formatContactTimestamp(createdAt)
        const currentSortDate = new Date(prev._sortDate || 0).getTime()
        const nextSortDate = createdAt.getTime()

        if (
          prev.lastMessage === nextLastMessage &&
          prev.timestamp === nextTimestamp &&
          currentSortDate === nextSortDate
        ) {
          return prev
        }
    ...
    const syncMessages = async ({ showLoader = false }: { showLoader?: boolean } = {}) => {
      if (showLoader) {
      ...
      try {
        ...
        if (!isActive || !result.success || !result.messages) {
          return
        }

        const serverMessages = result.messages.map((msg) => mapServerMessageToUi(msg, user.id))
        const bufferedMessages = initialFetchCompleted
          ? []
          : bufferedEvents.map((event) => mapRealtimeMessageToUi(event, user.id))

        mergeMessages([...serverMessages, ...bufferedMessages])

        const latestMessage = serverMessages[serverMessages.length - 1]
        if (latestMessage && latestMessage.id !== latestContactMessageIdRef.current) {
          latestContactMessageIdRef.current = latestMessage.id
          updateContactPreview(latestMessage, latestMessage.createdAt)
        }

        initialFetchCompleted = true

        const hasUnreadFromOther = result.messages.some(
    ...
    const applyIncomingMessage = (data: any) => {
      ...
      const incomingMessage = mapRealtimeMessageToUi(data, user.id)
      mergeMessages([incomingMessage])
      latestContactMessageIdRef.current = incomingMessage.id

      const incomingDate = new Date(data.createdAt)
      updateContactPreview(incomingMessage, incomingDate)

      void markMessagesAsRead(selectedConversationId)
        .then(() => {
          if (isActive) {
            markConversationReadLocally()
          }
    ...
    if (pusher) {
      ...
      chatChannel.bind("pusher:subscription_error", (status: number) => {
        console.error("Chat realtime subscription failed:", status)
        realtimeChatUnavailableRef.current = true

        if (chatChannel && pusher) {
          chatChannel.unbind("new-message")
          chatChannel.unbind("pusher:subscription_error")
    ...
    void syncMessages({ showLoader: true })
  ...
  useEffect(() => {
    if (
      !user?.id ||
      !isRealtimeMessagingFeatureEnabled() ||
      realtimeMessagingUnavailableRef.current
    ) {
      return
    }
    ...
    const messagingChannelName = buildMessagingChannel(user.id)
    ...
    const onConversationUpdated = (data: any) => {
      ...
      const eventDate = data.createdAt ? new Date(data.createdAt) : null
      ...
      setContacts((prev) => {
        ...
        const updated = prev.map((contact) => {
          ...
          const nextLastMessage =
            typeof data.lastMessagePreview === "string"
              ...
              : contact.lastMessage
          const nextTimestamp = eventDate ? formatContactTimestamp(eventDate) : contact.timestamp
          const nextUnreadCount =
            typeof data.conversationUnreadCount === "number"
              ? data.conversationUnreadCount
              : contact.unreadCount
          const nextSortDate = eventDate || contact._sortDate

          const currentSortDate = new Date(contact._sortDate || 0).getTime()
          const incomingSortDate = new Date(nextSortDate || 0).getTime()

          if (
            contact.lastMessage === nextLastMessage &&
            contact.timestamp === nextTimestamp &&
            contact.unreadCount === nextUnreadCount &&
            currentSortDate === incomingSortDate
          ) {
            return contact
          }
        ...
        return hasPreviewUpdate ? sortContactsByRecency(updated) : updated
      })
        ...
        return {
          ...
          lastMessage:
            typeof data.lastMessagePreview === "string"
              ? data.lastMessagePreview
              : prev.lastMessage,
          timestamp: eventDate ? formatContactTimestamp(eventDate) : prev.timestamp,
          unreadCount:
            typeof data.conversationUnreadCount === "number"
              ? data.conversationUnreadCount
              : prev.unreadCount,
          _sortDate: eventDate || prev._sortDate,
        }
      })
    }

    messagingChannel.bind("conversation-updated", onConversationUpdated)
    const onSubscriptionError = (status: number) => {
      console.error("Messaging realtime subscription failed:", status)
      realtimeMessagingUnavailableRef.current = true
      messagingChannel.unbind("conversation-updated", onConversationUpdated)
      messagingChannel.unbind("pusher:subscription_error", onSubscriptionError)
      pusher.unsubscribe(messagingChannelName)
    }
    messagingChannel.bind("pusher:subscription_error", onSubscriptionError)
  ...
  useEffect(() => {
    const end = Math.min(filteredContacts.length, 30)
    setContactWindow({ start: 0, end })
  }, [filteredContacts.length])

  useEffect(() => {
    const end = filteredMessages.length
    const start = Math.max(0, end - 120)
    setMessageWindow({ start, end })
  }, [filteredMessages.length, selectedConversationId, conversationSearch])

  const visibleContacts = useMemo(
    () => filteredContacts.slice(contactWindow.start, contactWindow.end),
    [filteredContacts, contactWindow.start, contactWindow.end]
  )

  const visibleMessages = useMemo(
    () => filteredMessages.slice(messageWindow.start, messageWindow.end),
    [filteredMessages, messageWindow.start, messageWindow.end]
  )

  const contactTopSpacerHeight = contactWindow.start * CONTACT_ITEM_ESTIMATED_HEIGHT
  const contactBottomSpacerHeight = Math.max(
    0,
    (filteredContacts.length - contactWindow.end) * CONTACT_ITEM_ESTIMATED_HEIGHT
  )
  const messageTopSpacerHeight = messageWindow.start * MESSAGE_ITEM_ESTIMATED_HEIGHT
  const messageBottomSpacerHeight = Math.max(
    0,
    (filteredMessages.length - messageWindow.end) * MESSAGE_ITEM_ESTIMATED_HEIGHT
  )
  ...
  const handleContactListScroll = () => {
    ...
    const visibleCount = Math.ceil(container.clientHeight / CONTACT_ITEM_ESTIMATED_HEIGHT)
    const nextStart = Math.max(
      0,
      Math.floor(container.scrollTop / CONTACT_ITEM_ESTIMATED_HEIGHT) - CONTACT_OVERSCAN
    )
    const nextEnd = Math.min(
      filteredContacts.length,
      nextStart + visibleCount + CONTACT_OVERSCAN * 2
    )
  ...
  const handleMessageListScroll = () => {
    ...
    const visibleCount = Math.ceil(container.clientHeight / MESSAGE_ITEM_ESTIMATED_HEIGHT)
    const nextStart = Math.max(
      0,
      Math.floor(container.scrollTop / MESSAGE_ITEM_ESTIMATED_HEIGHT) - MESSAGE_OVERSCAN
    )
    const nextEnd = Math.min(
      filteredMessages.length,
      nextStart + visibleCount + MESSAGE_OVERSCAN * 2
    )
          ...
          <div className="md:col-span-1 border rounded-lg bg-card overflow-hidden flex flex-col">
            ...
            >
              {loadingConversations ? (
                ...
                <>
                  {contactTopSpacerHeight > 0 ? <div style={{ height: `${contactTopSpacerHeight}px` }} /> : null}
                  ...
                  {contactBottomSpacerHeight > 0 ? <div style={{ height: `${contactBottomSpacerHeight}px` }} /> : null}
                </>
              )}
            </div>
          </div>
              ...
              <>
                ...
                >
                  {loadingMessages ? (
                    ...
                    <>
                      {messageTopSpacerHeight > 0 ? <div style={{ height: `${messageTopSpacerHeight}px` }} /> : null}
                      {visibleMessages.map((msg, index) => {
                        const globalIndex = messageWindow.start + index
                        // Check if we need to show a date separator
                        const showDateSeparator = globalIndex === 0 ||
                          new Date(filteredMessages[globalIndex - 1].createdAt).toDateString() !== new Date(msg.createdAt).toDateString()
                      ...
                      {messageBottomSpacerHeight > 0 ? <div style={{ height: `${messageBottomSpacerHeight}px` }} /> : null}
